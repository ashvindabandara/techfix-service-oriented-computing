...
namespace TechFixFrontend
{
    public partial class PlaceOrder : Form
    {
        ...
        private async void submit_Click(object sender, EventArgs e)
        {
            ...
            var apiUrl = "https://localhost:44346/api/Supplier/PlaceOrder";
            ...
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        MessageBox.Show("Order request sent successfully.\n" + result);
                    }
                    else
                    {
                        MessageBox.Show($"Error placing order. Status: {response.StatusCode}");
                    }
                }
                catch (Exception ex)
                {
